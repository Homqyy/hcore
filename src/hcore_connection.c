...
void
hcore_destroy_connection(struct hcore_connection_s *c)
{
    if (c->shared) { c->fd = -1; }
    else if (c->fd != -1)
    {
        ...
        if (0 < n)
        {
            ...
            if (c->bind_peer && !c->socklen)
            {
                ...
                *c->sockaddr = sockaddr;
                ...
                if (c->addr_text.data == NULL) { return HCORE_FAILED; }
...
hcore_tcp_send(struct hcore_connection_s *c, hcore_uchar_t *buf, size_t size)
{
    ...
    for (;;)
    {
        ...
        if (err == EAGAIN || err == EINTR)
        {
            ...
            if (err == EAGAIN) { return HCORE_AGAIN; }
        }
        else
        {
            wev->error = 1;
