...
typedef struct
{
    ...
    hcore_array_t *revs; // events for read. the structure is pki_event_t
    hcore_array_t *wevs; // events for write. the structure is pki_event_t

    struct timespec timeout; // timeout for processing event
    sigset_t
        sigmask; // signal mask is substitue of original when processing event
